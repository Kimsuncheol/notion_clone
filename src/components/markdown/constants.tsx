...
export const componentsForAIChat: Components = {
  // Custom components for better styling
  h1: ({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) => (
    <h1 className="text-gray-700 text-4xl font-bold dark:text-gray-300 leading-relaxed" style={style}>{children}</h1>
  ),
  ...
  code: (props: React.ComponentProps<'code'> & { inline?: boolean }) => {
    ...
    return inline ? (
      <code className="px-1 py-0.5 rounded text-sm font-mono bg-amber-50" style={{ backgroundColor: 'transparent'}} {...rest} >
        {children}
      </code>
    ) : (
      <code className="block p-4 rounded-lg text-sm font-mono overflow-x-auto whitespace-pre-line bg-amber-50" style={{ backgroundColor: 'transparent'}} {...rest}>
        {children}
      </code>
    );
  },
