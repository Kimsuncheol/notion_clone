...
const sidebarSlice = createSlice({
  ...
  reducers: {
    ...
    movePageToTrash: (state, action: PayloadAction<{ noteId: string; subNoteId?: string; title: string }>) => {
      const { noteId, subNoteId, title } = action.payload;
      console.log("movePageToTrash in sidebarSlice", noteId, subNoteId, title);
      // Find and remove the note from its current folder, preserving sub-notes data
      let existingNote: NoteNode | null = null;
      for (const folder of state.folders) {
        const noteIndex = folder.notes.findIndex(n => n.id === noteId);
